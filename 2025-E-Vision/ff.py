...
hires_mode = False           # 高分辨模式，适合 find_circle 模式使用，帧率会更低但是找圆圈更精准
                             # 不 find_circle 也可以使用，找4个角点更精准，需要配合设置合理的 std_res
                             # 注意开启了这个模式，输出的误差值也是基于大图的分辨率
high_res = 240               # 高分辨率模式宽高,越高越清晰但是帧率越低，注意 std_res 也要跟着改大点
...
cam_buff_num = 1             # 摄像头缓冲， 1 延迟更低帧率慢一点点， 2延迟更高帧率更高一点点
...
serial1 = uart.UART("/dev/ttyS1", 115200)  # 使用115200波特率
###################################################################################
...
def calculate_checksum(data_bytes):
    """
    计算给定字节序列的简单累加和校验和（取低8位）。
    """
    checksum = sum(data_bytes) & 0xFF  # 累加所有字节并取低8位
    return checksum

def send_error_data(err_x, err_y):
    """
    发送误差数据到下位机
    数据包格式: 帧头(2字节) + 控制字(1字节) + 数据长度(1字节) + X误差(2字节) + Y误差(2字节) + 校验和(1字节)
    0xFF 0xFE 0x55 0x04 X_H X_L Y_H Y_L checksum
    """
    # 将浮点数转为整数并加上224
    x_int = int(err_x) + 224
    y_int = int(err_y) + 224
    ...
    x_int = max(min(x_int, 65535), 0)
    y_int = max(min(y_int, 65535), 0)
    
    # 组装协议包
    header_part = bytes([0xFF, 0xFE])
    control_and_len_part = bytes([0x55, 0x04])  # 0x55是控制字，0x04是固定数据长度
    data_part = struct.pack('>HH', x_int, y_int)  # 大端序打包两个uint16
    
    # 计算校验和（包含帧头、控制字、数据长度和数据部分）
    payload_for_checksum = header_part + control_and_len_part + data_part
    checksum = calculate_checksum(payload_for_checksum)
    checksum_part = bytes([checksum])
    
    # 组装完整的包
    full_packet = payload_for_checksum + checksum_part
    
    # 发送数据
    serial1.write(full_packet)
...
# 在主循环前添加跳帧相关变量
frame_skip = 2  # 每2帧处理一次AI检测
frame_count = 0
last_objs = []  # 保存上一次的AI检测结果
last_max_idx = -1  # 保存上一次的最大目标索引
...
target_detected = False  # 添加目标检测标志
...
while not app.need_exit():
    ...
    frame_count += 1
    target_detected = False  # 每帧开始时重置检测标志
    ...
    if frame_count % frame_skip == 0:
        ...
        if hires_mode:
            ...
            img_ai = img # new copy
        debug_time("resize (skip AI)")
        
        objs = last_objs
        max_idx = last_max_idx
        debug_time("use last detect result")
    ...
    if max_idx >= 0:
                ...
                if minH > rect_min_limit and minW > rect_min_limit:
                    ...
                    if circle_dist > 0:
                        ...
                        if (not find_circle) or (center_new):
                            ...
                            # 发送误差数据到下位机
                            send_error_data(err_center[0], err_center[1])
                            target_detected = True  # 标记已检测到目标
                            debug_time("send uart")
        ...
        # 画AI检测框
        # img_ai.draw_rect(obj.x, obj.y, obj.w, obj.h, color = image.COLOR_GREEN, thickness=2)
        debug_time("draw")
    
    # 如果没有检测到有效目标，发送固定误差值666,666
    if not target_detected:
        send_error_data(666, 666)
        debug_time("send default error")
    ...
    if DEBUG or debug_show_hires:
        ...
        if debug_draw_err_msg:
            img.draw_string(2, img.height() - 32, f"err: {err_center[0]:5.1f}, {err_center[1]:5.1f}, fps: {time.fps():2.0f}", image.COLOR_RED, scale=1.0, thickness=1)
        disp.show(img)
    else:
        if debug_draw_err_line:
            img_ai.draw_line(center_pos[0], center_pos[1], last_center_small[0], last_center_small[1], image.COLOR_RED, thickness=3)
        if debug_draw_err_msg:
            img_ai.draw_string(2, img.height() - 32, f"err: {err_center[0]:5.1f}, {err_center[1]:5.1f}, fps: {time.fps():2.0f}", image.COLOR_RED, scale=1.0, thickness=1)
        disp.show(img_ai)
