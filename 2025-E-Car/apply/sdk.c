...
作者:	无名创新
****************************************************/
void position_control(float fixed_threshold_cm,uint16_t feed_times)//已任意姿态到达目标点,无速度末端角度约束
{
  ...
  distance_ctrl.error=constrain_float(distance_ctrl.error,-50,50);
	...
	//航点遍历结束标志判断
	if(ngs_nav_ctrl.cnt<feed_times)//持续50ms满足
	{
...
void distance_control_with_speed_limit(float speed_limit)
{
  ...
  distance_ctrl.error=constrain_float(distance_ctrl.error,-100,100);
	...
	distance_ctrl.output=constrain_float(distance_ctrl.output,-speed_limit,speed_limit);//对速度期望进行限制，避免轮胎打滑造成里程计误差
}